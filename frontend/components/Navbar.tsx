...
const Navbar = () => {
  ...
  return (
    <header
      className='fixed top-0 left-0 right-0 z-50 px-4 sm:px-6 lg:px-8 bg-slate-50 md:bg-transparent'
      style={{
          ...
          <div className='hidden md:flex items-center justify-between w-full px-6 relative z-10'>
            ...
            <div className='flex justify-center px-4'>
              <Link href='/' className='relative group'>
                <div className='absolute -inset-3 bg-yellow-400/20 rounded-full blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500'></div>
                <div className='relative h-[56px] w-[56px] transition-transform duration-500 group-hover:scale-110'>
                  <Image
                    src='/logo.png'
            ...
            {/* Right Links */}
            <div className='flex items-center gap-x-6 flex-1 justify-start pl-3'>
          ...
          <div className='md:hidden flex justify-between items-center w-full px-4 relative z-10 py-1'>
            ...
            >
              <div className='w-6 h-6 relative flex items-center justify-center'>
                <span
                  className={`absolute block w-full h-0.5 bg-current transition-all duration-300 ease-in-out ${isOpen ? 'rotate-45 translate-y-0' : '-translate-y-2'}`}
                ></span>
                <span
                  className={`absolute block w-full h-0.5 bg-current transition-all duration-300 ease-in-out ${isOpen ? 'opacity-0' : 'opacity-100'}`}
                ></span>
                <span
                  className={`absolute block w-full h-0.5 bg-current transition-all duration-300 ease-in-out ${isOpen ? '-rotate-45 translate-y-0' : 'translate-y-2'}`}
                ></span>
          ...
          <div className='bg-blue-950 border-t border-white/10 shadow-2xl p-4 space-y-1'>
            {navLinks.map((link) => (
              ...
              >
                ...
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center border border-white/10 group-hover:border-yellow-400/50 group-hover:bg-yellow-400/10 transition-all duration-300`}
                >
                  <svg
...
const NavLink = ({
  ...
  <Link
    href={link.href}
    className={`relative font-bold tracking-[0.15em] transition-all duration-300 uppercase text-sm group py-1
